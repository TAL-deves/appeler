              ...
              return GestureDetector(
                onTap: () {
                  if (!isOnline) {
                    ...
                    AppAlertDialog.outGoingCallDialog(
                            context: context, callerId: id)
                        .then((value) {
                      if (value != null && value) {
                        remoteUser.update(
                            {'incomingCallFrom': null, 'inAnotherCall': false});
                        curUser.update({
                          'inAnotherCall': false,
                        });
                      }
                    ...
                    final curRoom =
                        chatRooms.doc('${AuthManagementUseCase.curUser}+$id');
                    subscription?.cancel();
                        ...
                        if (isAccepted != null) {
                          if (isAccepted) {
                            ...
                            Navigator.of(context).pushNamed(callingScreenRoute,
                                arguments: [id, CallEnum.outgoing]);
                          } else {
                            curRoom.delete();
                            curUser.update({
                              'inAnotherCall': false,
